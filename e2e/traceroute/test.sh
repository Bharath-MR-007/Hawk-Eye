...
{
    kathara lclean
    yes | rm ./shared/api.json ./shared/prometheus.txt ./shared/hawkeye
    exit $EXIT_CODE
}
...
function check_prometheus_output() {
  if grep -q 'hawkeye_traceroute_minimum_hops{target="200.1.1.7"} 3' ./shared/prometheus.txt; then
...
info "Using $HAWKEYE_BIN"
cp $HAWKEYE_BIN ./shared/hawkeye

# Start Hawk-Eye on pc1
kathara exec pc1 "/shared/hawkeye run --config /shared/config.yaml" &

# Wait for 10 seconds to ensure Hawk-Eye is up and running
sleep 10

# Curl the API of Hawk-Eye
