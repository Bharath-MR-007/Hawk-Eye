...
# Hawk-Eye - Infrastructure Monitoring<!-- omit from toc -->

<!-- markdownlint-disable MD033 -->
<p align="center">
  <a href="/../../commits/" title="Last Commit"><img alt="Last Commit" src="https://img.shields.io/github/last-commit/Bharath-MR-007/Hawk-Eye?style=flat"></a>
  <a href="/../../issues" title="Open Issues"><img alt="Open Issues" src="https://img.shields.io/github/issues/Bharath-MR-007/Hawk-Eye?style=flat"></a>
...
The `hawkeye` is an infrastructure monitoring tool. The binary includes several checks (e.g. health check) that will be
executed periodically.

## About this component

The `hawkeye` performs several checks to monitor the health of the infrastructure and network from its point of view.
The following checks are available:

1. [Health check](#check-health) - `health`: The `hawkeye` is able to perform an HTTP-based (HTTP/1.1) health check to
   the provided endpoints. The `hawkeye` will expose its own health check endpoint as well.

2. [Latency check](#check-latency) - `latency`: The `hawkeye` is able to communicate with other `hawkeye` instances to
   calculate the time a request takes to the target and back. The check is http (HTTP/1.1) based as well.

3. [DNS check](#check-dns) - `dns`: The `hawkeye` is able to perform DNS resolution checks to monitor domain name system
   performance and reliability. The check has the ability to target specific domains or IPs for monitoring.

4. [Traceroute Check](#check-traceroute) - `traceroute`: The `hawkeye` is able to perform traceroute checks to monitor
...
The `hawkeye` is provided as a small binary & a container image.

Please refer to the [release notes](https://github.com/Bharath-MR-007/Hawk-Eye/releases) to get the latest version.
...
```sh
curl https://github.com/Bharath-MR-007/Hawk-Eye/releases/download/v${RELEASE_VERSION}/hawkeye_${RELEASE_VERSION}_linux_amd64.tar.gz -Lo hawkeye.tar.gz
curl https://github.com/Bharath-MR-007/Hawk-Eye/releases/download/v${RELEASE_VERSION}/hawkeye_${RELEASE_VERSION}_checksums.txt -Lo checksums.txt
```

Extract the binary:

```sh
tar -xf hawkeye.tar.gz
```

### Container Image

The [hawkeye container images](https://github.com/Bharath-MR-007/Hawk-Eye/pkgs/container/hawkeye) for
dedicated [release](https://github.com/Bharath-MR-007/Hawk-Eye/releases) can be found in the GitHub registry.

### Helm

Hawk-Eye can be installed via Helm Chart. The chart is available in the GitHub registry:

```sh
helm -n hawkeye upgrade -i hawkeye oci://ghcr.io/Bharath-MR-007/charts/hawk-eye --create-namespace
```

The default settings are suitable for a local configuration. With the default Helm values, the hawkeye loader uses a
...
To provide the hawkeye container with the token, manually create a secret containing the `HAWKEYE_LOADER_HTTP_TOKEN`
environment variable. Utilize the `envFromSecrets` in the `values.yaml` to enable access to this secret by the hawkeye
container. Avoid adding sensitive data like the token used by the `http` loader (`loader.http.token`) directly in the
values section.

The same applies to the target manager token. Use the `HAWKEYE_TARGETMANAGER_GITLAB_TOKEN` in a secret and bind it with
the `envFromSecrets` in the `values.yaml`.

For all available value options see [Chart README](./chart/README.md).

Additionally check out the hawkeye [configuration](#configuration) variants.

## Usage

Use `hawkeye run` to execute the instance using the binary. A `hawkeyeName` (a valid DNS name) is required to be passed,
else the hawkeye will not start:

```sh
hawkeye run --hawkeyeName hawkeye.telekom.de
```

### Image

Run a `hawkeye` container by using e.g. `docker run ghcr.io/Bharath-MR-007/Hawk-Eye`.

Pass the available configuration arguments to the container e.g. `docker run ghcr.io/Bharath-MR-007/Hawk-Eye --help`.

Start the instance using a mounted startup configuration file
e.g. `docker run -v /config:/config  ghcr.io/Bharath-MR-007/Hawk-Eye --config /config/config.yaml`.

## Configuration

The configuration is divided into two parts. The startup configuration and the checks' configuration. The startup
configuration is a technical configuration to configure the `hawkeye` instance itself.

### Startup

The available configuration options can be found in the [CLI flag documentation](docs/hawkeye.md).

The `hawkeye` is able to get the startup configuration from different sources as follows.
...
```bash
export HAWKEYE_LOADER_HTTP_TOKEN="xxxxxx"
```

Or for any other config attribute:

```bash
export HAWKEYE_ANY_OTHER_OPTION="Some value"
```
...
```yaml
# DNS hawkeye is exposed on 
name: hawkeye.example.com
...
loader:
  ...
  type: http
  # The interval in which hawkeye tries to fetch a new configuration
...
# Configures the API
api:
  # Which address to expose Hawk-Eye's REST API on
  address: :8080
...
targetManager:
  ...
  # Scheme defines with which scheme hawkeye should register itself
  scheme: http
  ...
  gitlab:
    ...
    # You can also set this value through the HAWKEYE_TARGETMANAGER_GITLAB_TOKEN environment variable
    token: glpat-xxxxxxxx
    # The ID of your GitLab project. This is where Hawk-Eye will register itself
    # and grab the list of other Hawk-Eyes from
    projectId: 18923
...
The loader component of the `hawkeye` dynamically loads the [checks](#checks)' configuration during runtime.
...
You can configure the logging behavior of the hawkeye instance by setting the following environment variables:
...
In addition to the technical startup configuration, the `hawkeye` checks' configuration can be dynamically loaded during runtime.
...
to [this documentation](docs/hawkeye_run.md).
...
The `hawkeye` can optionally manage targets for checks and register itself as a target on a (remote) backend through
...
| `targetManager.registrationInterval` | Interval for registering the current hawkeye at the target backend. 0 means no registration.                                                    |
| `targetManager.updateInterval`       | Interval for updating the registration of the current hawkeye. 0 means no update.                                                               |
...
backend. The various `hawkeye` instances can register themselves as targets in the project.
The `hawkeye` instances will also check the project for new targets and add them to the local state.
The registration is done by committing a "state" file in the main branch of the repository,
which is named after the DNS name of the `hawkeye`. The state file contains the following information:

```json
{
  "url": "<SCHEME>://<HAWKEYE_DNS_NAME>",
...
| `targets`     | `list of strings` | List of targets to send health probe. Needs to be a valid URL. Can be another `hawkeye` instance. Automatically updated when a targetManager is configured. |
...
| `targets`     | `list of strings` | List of targets to send latency probe. Needs to be a valid URL. Can be another `hawkeye` instance. Automatically updated when a targetManager is configured. |
...
- `hawkeye_latency_duration_seconds`
  - Type: Gauge
  - Description: Latency with status information of targets. This metric is DEPRECATED. Use `hawkeye_latency_seconds`.
  - Labelled with `target` and `status`

- `hawkeye_latency_seconds`
  - Type: Gauge
  - Description: Latency information of targets
  - Labelled with `target`

- `hawkeye_latency_count`
  - Type: Counter
  - Description: Count of latency checks done
  - Labelled with `target`

- `hawkeye_latency_duration`
  - Type: Histogram
...
| `targets`     | `list of strings` | List of targets to lookup. Needs to be a valid domain or IP. Can be another `hawkeye` instance. Automatically updated when a targetManager is configured. |
...
Hawk-Eye does not need any extra permissions to run this check. However, some data, like the ip address
of the hop that dropped a packet, will not be available. To enable this functionality, there are two options:

- Run hawkeye as root:

  ```bash
  sudo hawkeye run --config config.yaml
  ```

- Allow hawkeye to create raw sockets, by assigning the `CAP_NET_RAW` capability to the hawkeye binary:

  ```bash
  sudo setcap 'cap_net_raw=ep' hawkeye
  ```

#### Traceroute Prometheus Metrics

- `hawkeye_traceroute_check_duration_ms{target="google.com"} 43150`
  - Type: Gauge
  - Description: How long the last traceroute took for this target in total
- `hawkeye_traceroute_minimum_hops{target="google.com"} 14`
  - Type: Gauge
...
The `hawkeye` provides a `/metrics` endpoint to expose application metrics. In addition to runtime information, the hawkeye provides specific metrics for each check. Refer to the [Checks](#checks) section for more detailed information.

### Prometheus Integration

The `hawkeye` metrics API is designed to be compatible with Prometheus. To integrate `hawkeye` with Prometheus, add the following scrape configuration to your Prometheus configuration file:

```yaml
scrape_configs:
  - job_name: 'hawkeye'
    static_configs:
      - targets: ['<hawkeye_instance_address>:8080']
```

Replace `<hawkeye_instance_address>` with the actual address of your `hawkeye` instance.

### Traces

The `hawkeye` supports exporting telemetry data using the OpenTelemetry Protocol (OTLP). This allows users to choose their preferred telemetry provider and collector. The following configuration options are available for setting up telemetry:
...
| Type       | Channel                                                                                                                                                           |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Issues** | <a href="/../../issues/new/choose" title="General Discussion"><img alt="Issues" src="https://img.shields.io/github/issues/Bharath-MR-007/Hawk-Eye?style=flat-square"></a> |
<!-- markdownlint-enable MD033 -->
