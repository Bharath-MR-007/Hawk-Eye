...
type ChecksController struct {
	...
	// pm publishes check results to WebSocket subscribers
	pm *probes.ProbeManager
}
...
// WithProbeManager attaches a ProbeManager for WebSocket live streaming.
func (cc *ChecksController) WithProbeManager(pm *probes.ProbeManager) {
	cc.pm = pm
}
...
func (cc *ChecksController) Run(ctx context.Context) error {
	...
	for {
		...
		case result := <-cc.cResult:
			cc.db.Save(result)
			// Publish to WebSocket subscribers if a ProbeManager is wired up
			if cc.pm != nil {
				cc.pm.Publish(&probes.ProbeResult{
...
var oapiBoilerplate = openapi3.T{
	...
	OpenAPI: "3.0.0",
	Info: &openapi3.Info{
		Title:       "Hawkeye Metrics API",
		Description: "Serves metrics collected by hawkeyes checks",
