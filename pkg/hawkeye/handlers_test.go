...
func TestHawkeye_handleOpenAPI(t *testing.T) {
	s := Hawkeye{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := tt.decoder(tt.args.response); err != nil {
				t.Errorf("failed to decode response Hawkeye.getOpenapi() = %v", err)
			}

			if tt.args.response.Code != http.StatusOK {
				t.Errorf("Hawkeye.getOpenapi() = %v, want %v", tt.args.response.Code, http.StatusOK)
			}
		})
	}
}

func TestHawkeye_handleCheckMetrics(t *testing.T) {
	tests := []struct {
		name     string
		want     []byte
		wantCode int
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantCode == http.StatusOK {
				if tt.wantCode != resp.StatusCode {
					t.Errorf("Hawkeye.getCheckMetrics() = %v, want %v", resp.StatusCode, tt.wantCode)
				}
				...
				if reflect.DeepEqual(got, want) {
					t.Errorf("Hawkeye.getCheckMetrics() = %v, want %v", got, want)
				}
			} else {
				if tt.wantCode != resp.StatusCode {
					t.Errorf("Hawkeye.getCheckMetrics() = %v, want %v", resp.StatusCode, tt.wantCode)
				...
				if !reflect.DeepEqual(body, tt.want) {
					t.Errorf("Hawkeye.getCheckMetrics() = %v, want %v", body, tt.want)
				}
			}
		})
	}
