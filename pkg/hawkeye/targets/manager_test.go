...
// Test_gitlabTargetManager_registerHawkeye tests that the register method will
// register the hawkeye instance in the remote instance
...
// method will update the registration of the hawkeye instance in the remote instance
...
func Test_gitlabTargetManager_Reconcile_Registration_Update(t *testing.T) {
	...
	case <-timeout:
		...
		gtm.mu.Lock()
		if !gtm.registered {
			t.Fatalf("Reconcile() should have registered the hawkeye")
		}
		gtm.mu.Unlock()
	...
	t.Logf("Reconcile() successfully registered and updated the hawkeye")
}
