...
To run hawkeye we first need to build and move the hawkeye binary into the container. Luckily, kathara mounts a shared folder to all systems in the lab.
We can use this folder to run hawkeye in the containers without having to build our own image!

```bash
go build -o hawkeye . && mv hawkeye pkg/checks/traceroute/test-lab/shared/
```
...
```bash
root@pc1:/# cd /shared
root@pc1:/shared# ./hawkeye -h
Hawk-Eye is an infrastructure monitoring agent that is able to perform different checks.
...
Flags:
  ...
  -h, --help            help for hawkeye

Use "hawkeye [command] --help" for more information about a command.
```

Now we just have to create a config for hawkeye to use and we're ready to develop. For testing traceroute I used this config:
...
Now just run hawkeye in the shared folder:

```bash
root@pc1:/shared# ./hawkeye run --config config.yaml
```
