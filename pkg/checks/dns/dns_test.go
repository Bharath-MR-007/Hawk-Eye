	...
	tests := []struct {
		...
		{ //nolint:dupl // normal lookup
			...
			mockSetup: func() *DNS {
				...
				c.client = &ResolverMock{
					LookupHostFunc: func(ctx context.Context, addr string) ([]string, error) {
						return []string{exampleIP, hawkeyeIP}, nil
					},
					SetDialerFunc: func(d *net.Dialer) {},
				}
				return c
			},
			targets: []string{exampleURL, hawkeyeURL},
			want: checks.Result{
				Data: map[string]result{
					exampleURL: {Resolved: []string{exampleIP, hawkeyeIP}},
					hawkeyeURL: {Resolved: []string{exampleIP, hawkeyeIP}},
				},
			},
		},
			...
			mockSetup: func() *DNS {
				...
				c.client = &ResolverMock{
					LookupAddrFunc: func(ctx context.Context, addr string) ([]string, error) {
						return []string{exampleURL, hawkeyeURL}, nil
					},
					SetDialerFunc: func(d *net.Dialer) {},
				}
				return c
			},
			targets: []string{exampleIP, hawkeyeIP},
			want: checks.Result{
				Data: map[string]result{
					exampleIP: {Resolved: []string{exampleURL, hawkeyeURL}},
					hawkeyeIP: {Resolved: []string{exampleURL, hawkeyeURL}},
				},
			},
		},
		{
