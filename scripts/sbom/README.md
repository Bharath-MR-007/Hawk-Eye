...
```shell
SYFT_GOLANG_SEARCH_REMOTE_LICENSES=true syft ghcr.io/Bharath-MR-007/hawk-eye:v0.5.0 -o template -t scripts/sbom/example.sbom.tmpl
```

Setting the env variable `SYFT_GOLANG_SEARCH_REMOTE_LICENSES=true` will ensure to lookup licenses remotely. In this example the hawkeye image in version `v0.5.0` is scanned.
